...
if int(text) == 0:
    ...
    ts = gTTS(text="Your mail has been sent. ", lang='en')
    tsname=("send.mp3")
    ts.save(tsname)
    music = pyglet.media.load(tsname, streaming = False)
    music.play()
...
if int(text) == 1:
    ...
    login_id.close()
    login_pass.close()
    send_id.close()                                          #this is host and port area.... ssl security
    unm = (login_name)                                                        #username
    psw = (login_pwd)                                                                         #password
    ...
    unseen = str(unseen[1])
    unseen = unseen.split()                                                        # unseen count
